 ...
 *
 */
public class Question3_BreadthFirstApproach {
	...
	private static LinkedList<LinkedList<Node>> createDepthList(Node root) {
		...
		if (root != null) {
			currentDepthList.add(root);
		}
		
		// Iterate through the current depth level and find the next depth level.
		while (currentDepthList.size() > 0) {
			LinkedList<Node> nextDepthList = new LinkedList<Node>();
			...
			// Adds the current depth list to the depth list.
			depthList.add(currentDepthList);
			
			// Set the current depth list to the next depth list.
